...
```
```{r}
p4 <- data %>% ggplot(aes(x = log(meat_production_tonnes) , y = urb2019, colour = division_idh)) + 
  geom_point() + 
...
```
```{r}
p5 <- data %>% ggplot(aes(x = meat_production_per_capita, y = gdpPerCapita, colour = division_idh)) + 
  geom_point() + 
  labs(title="Producción de carne per cápita vs PIB per cápita", x = "Producción de carne per cápita", y="PIB per cápita") + 
  theme_bw()
p5
```
