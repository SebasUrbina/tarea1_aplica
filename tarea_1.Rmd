...
data <- merge(data_5, select(religion_data,Pais,christians_100,unaffiliated), by = "Pais", all.x = TRUE)
data$meat_production_per_capita <- data$meat_production_tonnes/data$pop2020
data$unaffiliated_100 <- data$unaffiliated/100000
data <- na.omit(data)
```
...
```
```{r}
data %>% ggplot(aes(x = log(meat_production_tonnes) , y = log(unaffiliated), colour = division_idh)) + 
  geom_point(size = 2) + 
  labs(title="log producción de carne vs log de no afiliados", x = "log producción de carne", y="log de no afiliados")+ theme_bw()
