...
religion_data$christians_100 <- (religion_data$chistians/religion_data$pop2020)*100000
christians_100 <- religion_data %>% select(Pais, christians_100)
religion_data$unaffiliated_100 <- (religion_data$unaffiliated/religion_data$pop2020)*100000
religion_data$hindus_100 <- (religion_data$hindus/religion_data$pop2020)*100000
religion_data$buddhists_100 <- (religion_data$buddhists/religion_data$pop2020)*100000
#Arreglo del IDH
...
data <- merge(data_5, select(religion_data,Pais,christians_100,unaffiliated_100,hindus_100,buddhists_100), by = "Pais", all.x = TRUE)
...
```{r}
dataxd <- na.omit(data) %>% select(pop2020, MedianAge, Meat_kg_2017,gdpPerCapita)
cor(dataxd)
```
...
p1 <- data %>% ggplot(aes(x = log(meat_production_tonnes) , y = christians_100, colour = division_idh)) + 
  geom_point(size = 2) + 
  labs(title="log producción de carne vs Cristianos cada 100mil habitantes", x = "log producción de carne", y="Tasa de Cristianos cada 100mil hab")+ theme_bw()
p1
```
```{r}
data %>% ggplot(aes(x = log(meat_production_tonnes) , y = unaffiliated_100, colour = division_idh)) + 
  geom_point(size = 2) + 
  labs(title="log producción de carne vs  no afiliados", x = "log producción de carne", y="Tasa de No afiliados")+ theme_bw()

```

```{r}
data %>% ggplot(aes(x = log(meat_production_tonnes) , y = log(buddhists_100), colour = division_idh)) + 
  geom_point(size = 2) + 
  labs(title="log producción de carne vs log tasa de budistas c/100khab", x = "log producción de carne", y="Tasa de budistas")+ theme_bw()

```

```{r}
data %>% ggplot(aes(x = log(meat_production_tonnes) , y = log(hindus_100), colour = division_idh)) + 
  geom_point(size = 2) + 
  labs(title="log producción de carne vs log tasa de hindus cada 100mil hab.", x = "log producción de carne", y="log Tasa de budistas cada 100mil hab.")+ theme_bw()
...
```
```{r}
p5 <- data %>% ggplot(aes(x = log(meat_production_per_capita), y = gdpPerCapita, colour = division_idh)) + 
  geom_point() + 
